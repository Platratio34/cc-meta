...
---@class InventoryPeripheral CC Generic inventory peripheral
local InventoryPeripheral = {}

---Get the size of this inventory.
---@return integer size The number of slots in this inventory.
...
---More information can be fetched with `getItemDetail`.
...
---
---@return { integer: InventoryPeripheral.Item } items All items in this inventory.
function InventoryPeripheral.list() end

---The returned information contains the same information as each item in `list`,
---     as well as additional details like the display name (`displayName`), and item and item durability (`damage`, `maxDamage`, `durability`).
---
---Some items include more information (such as enchantments) -
---     it is recommended to print it out using `textutils.serialize` or in the Lua REPL, to explore what is available.
---
---@param slot integer The slot to get information about.
---@return InventoryPeripheral.ItemDetail|nil item Information about the item in this slot, or `nil` if not present.
function InventoryPeripheral.getItemDetail(slot) end

---Get the maximum number of items which can be stored in this slot.
---
---Typically this will be limited to 64 items.
---However, some inventories (such as barrels or caches) can store hundreds or thousands of items in one slot.
---
...
---@return integer transfered The number of transferred items.
...
---@class InventoryPeripheral.Item
---@field name string
---@field count integer
---@field nbt string?

---@class InventoryPeripheral.ItemDetail : InventoryPeripheral.Item