...
---@return boolean successful Whether the program exited successfully.
...
---Otherwise they will be searched for from the current directory, rather than the computer's root.
---
...
---@param path string The name of the program
...
---@return { string: fun(shell: table, index: number, argument: string, previous: string[]):string[] } completions A table mapping the absolute path of programs, to their completion functions.
...
---@param alias string The name or path to the program
...
---
---@return { string: string } aliases A table, where the keys are the names of aliases, and the values are the path to the program.
...
---@return integer id The ID of the newly created tab
