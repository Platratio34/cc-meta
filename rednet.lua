...
rednet = {  ---CC's native network module
    CHANNEL_BORADCAST = 65535, ---The channel used by the Rednet API to broadcast messages.
...
---@return boolean open If the given modem is open.
...
---
---@param message any message to send. This should not contain coroutines or functions, as they will be converted to `nil`.
---@param protocol? string The "protocol" to send this message under. When using `rednet.receive` one can filter to only receive messages sent under a particular protocol.
...
---@return number|nil origin The computer which sent this message **OR** `nil` If the timeout elapsed and no message was received.
---@return any|nil message The received message
...
---the registered system will automatically respond via a background process, hence providing the system performing the lookup with its ID number.
---
---Multiple computers may not register themselves on the same network as having the same names against the same protocols,
---and the title localhost is specifically reserved.
---They may, however, share names as long as their hosted protocols are different,
---or if they only join a given network after "registering" themselves before doing so (eg. while offline or part of a different network).
---
...
---@return nil|number... hosts A list of computer IDs hosting the given protocol. **OR** `nil` if none exists.
